...
export const MeetTemplate: FC<AgentTemplateProps> = ({ title }) => {
  ...
  const [timeDateFormatted, setTimeDateFormatted] = useState(
    moment().format("h:mm A • ddd, MMM DD")
  );
  useEffect(() => {
  ...
  return (
    <ThemeProvider theme={theme}>
      <AppBar position="static" sx={{ bgcolor: "transparent", boxShadow: 0 }}>
        <Toolbar sx={{ p: 0.8, px: "16px !important" }}>
          ...
          <Stack spacing={1} sx={{ marginLeft: "auto" }} direction="row">
            ...
            <Tooltip
              title="Google Apps"
              sx={{ ml: `${theme.spacing(3)} !important` }}
            >
              <IconButton>
                <AppsIcon />
              </IconButton>
            ...
            <Tooltip title="Google Account">
              <Avatar sx={{ width: 32, height: 32, my: "auto !important" }}>
                <PersonIcon />
              </Avatar>
            </Tooltip>
          </Stack>
      ...
      >
        <Grid
          item
          sx={{ maxWdith: "35rem", flexBasis: "35rem", my: "auto !important" }}
        >
          <Typography
            sx={{
