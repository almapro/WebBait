defmodule WebBait.Accounts do
  ...
  alias WebBait.Accounts.{User, UserToken}
  ...
  #        %UserToken{} <- Repo.one(query),
  #        {:ok, _} <- Repo.transaction(user_username_multi(user, user.username, context)) do
  #     :ok
  #   else
  #     _ -> :error
  #   end
